...
- `StoryScript.ts` — the complete narrative ("Das Lied von Aelindra"): intro, one-time zone-entry beats, stage-dependent NPC dialog overrides, boss intro/phase-3/farewell lines, epilogue, **and `SIDE_QUESTS`** (each quest carries its mini-story as offer/active/done/target dialogs). Story stage (`act1/act2/act3/done`) derives from main-quest flags via `getStoryStage`
- `constants.ts` — single source for tunables, `GAME_EVENTS` names, shop items, skills/synergies, **`MATERIALS`/`RECIPES`/`RESOURCE_NODE_COUNTS`** (crafting), zone metadata, loot tables
...
- **Quests**: linear main quest (kill 5 → find shield → beat boss) tracked in `MainScene` fields. Side quests (`SIDE_QUESTS` in `StoryScript.ts`) are a per-NPC dialog state machine in `MainScene`: progress values are `undefined` (not offered) → `0..goal-1` (active) → `-1` (rewarded, thanks pending) → `-2` (thanked). First talk shows `offerLines`, completion is automatic, next talk shows `doneLines` once. Kinds: gather (herb/wood/ore/essence via `addMaterial`), kill (`kill_dragon`/`kill_any` via `onEnemyDeath`), `talk_npc` (delivering a message; the target speaks `targetLines`). One quest per giver NPC. Crafter/shop NPCs show pending quest dialogs before opening their screens.
